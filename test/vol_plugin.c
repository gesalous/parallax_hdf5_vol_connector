...
static herr_t test_registration_by_value(void) {
  ...
  if ((is_registered = H5VLis_connector_registered_by_name(
           PARALLAX_VOL_CONNECTOR_NAME)) < 0)
    TEST_ERROR;
  if (true == is_registered)
    FAIL_PUTS_ERROR("VOL connector is inappropriately registered");

  /* Register the connector by value */
  if ((vol_id = H5VLregister_connector_by_value(PARALLAX_VOL_CONNECTOR_VALUE,
                                                H5P_DEFAULT)) < 0)
    TEST_ERROR;
...
static herr_t test_multiple_registration(void) {
  ...
  for (i = 0; i < N_REGISTRATIONS; i++) {
    if ((vol_ids[i] = H5VLregister_connector_by_name(
             PARALLAX_VOL_CONNECTOR_NAME, H5P_DEFAULT)) < 0)
      TEST_ERROR;
  }
...
static herr_t test_getters(void) {
  ...
  if ((vol_id_out = H5VLget_connector_id_by_name(PARALLAX_VOL_CONNECTOR_NAME)) <
      0)
    TEST_ERROR;
